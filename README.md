...
- **拼接图片** —— 纵向 / 横向 / 九宫格拼接，支持直接拖入整个文件夹（递归读取）、拖拽排序、按文件名自然排序；可调质量、最长边限制、间距 / 边距 / 圆角 / 背景色，输出 webP / mozJPEG / avif
- **九宫格切割** —— 居中裁切为正方形后等分切割，支持 3×3 / 2×2 / 4×4，输出 PNG / JPEG / webP，可一键打包 ZIP 下载
- **图片压缩** —— 批量拖入图片重新编码压体积，质量可调，支持 webP / mozJPEG / avif，多张时打包 ZIP
