...
- **拼接图片** —— 纵向 / 横向 / 九宫格拼接，支持直接拖入整个文件夹（递归读取其中的图片）、拖拽排序、按文件名自然排序，输出 webP / mozJPEG / avif
- **九宫格切割** —— 居中裁切为正方形后等分切割，支持 3×3 / 2×2 / 4×4，输出 PNG / JPEG，可一键打包 ZIP 下载
...
拖入文件夹时通过 `webkitGetAsEntry` 递归读取其中的图片文件；九宫格切割结果的「打包下载 ZIP」由纯 JS 生成 store 模式的 ZIP（PNG/JPEG 本身已是压缩格式，无需再压），单文件即可保存全部切块，**不依赖浏览器的多文件下载许可**。
...
拼接编码期间每秒刷新「已用时」，AVIF 这类较慢的编码不会再让人以为页面卡死。
...
- AVIF 编码较慢，大图可能需要数十秒（编码期间会显示已用时，请耐心等待）
